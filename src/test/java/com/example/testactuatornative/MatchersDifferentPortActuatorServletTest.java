...
@SpringBootTest(webEnvironment = RANDOM_PORT, properties = { "spring.main.web-application-type=SERVLET",
...
public class MatchersDifferentPortActuatorServletTest extends AbstractBaseWebClientTest {
	...
	public void matchersHealthEndpointRequest() {
		testClient.get().uri("/actuator/health").exchange().expectStatus().isUnauthorized().expectBody(Map.class)
				.consumeWith(result -> assertEquals(result.getStatus(), HttpStatusCode.valueOf(401)));
	}

	@Test
	public void matchersInfoEndpointRequest() {
		testClient.get().uri("/actuator/info").exchange().expectStatus().isUnauthorized().expectBody(Map.class)
				.consumeWith(result -> assertEquals(result.getStatus(), HttpStatusCode.valueOf(401)));
	}
	...
	public void matchersWhiteListRequest() {
		testClient.get().uri("/public-access/headers-servlet").exchange().expectStatus().isOk().expectBody(Map.class)
				.consumeWith(result -> assertThat(result.getResponseBody()).hasSize(1).containsKeys("headers"));
	}

	@Test
	public void matchers401Error() {
		testClient.get().uri("/headers").exchange().expectStatus().isUnauthorized().expectBody(Map.class)
				.consumeWith(result -> assertEquals(result.getStatus(), HttpStatusCode.valueOf(401)));
	}
